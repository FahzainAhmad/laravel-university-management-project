...
<body style="background-color:#0F0E0E;">
    ...
    <div class="main-div">
        ...
        <div class="data-content">
            <div class="heads">
                <span style="color:white;font-size:1.7rem;">Newland Student Database</span>
                <form action="">
                ...
                <a href="/addstud"><button class="btn btn-success">Add Student</button></a>
            </div>
            <table class="table table-striped table-dark" style="margin-top:10px;">
                ...
                <tbody>
                    @foreach ($users as $data)
                        <tr class="ttr">
                            <td><img src="/images/userspfp/{{ $data->pfp }}" alt="" width="40px"
                                    height="40px" style="border-radius:100px;"></td>
                            <td>{{ $data->id }}</td>
                            <td>{{ $data->name }}</td>
                            <td>{{ $data->email }}</td>
                            <td>{{ $data->dname }}</td>
                            ...
                            <td>{{ $data->div }}</td>
                            <td>
                                <a href="{{ url('edit', $data->id) }}"><button type="button"
                                        class="btn btn-primary">Update</button></a>
                                <a href="{{ url('delete', $data->id) }}"><button type="button"
                                        class="btn btn-danger">Delete</button></a>
                                <a href="{{ url('downloadreport', $data->id) }}"><button type="button"
            ...
            <div style="width:80%;height:5%;display:flex;flex-direction:row;">{{ $users->links() }}</div>
        </div>
