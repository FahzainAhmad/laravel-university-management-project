...
<body style="background-color: #252525;">
    <div class="div-block">
        ...
        <form method="POST" action="/edittt/{{$tt['id']}}" enctype="multipart/form-data" style="width:100%;">
            @csrf
            <div style="display:flex;flex-direction:column;width:20%;">
                <label for="div" class="textlabel">Division</label>
                <input type="text" name="div" class="tfield" value="{{$tt['div']}}" />
            </div>
            <div style="display:flex;flex-direction:column;width:20%;">
                <label for="sem" class="textlabel">Semester</label>
                <input type="text" name="sem" class="tfield" value="{{$tt['sem']}}" />
            </div>
            <div style="display:flex;flex-direction:column;width:100%;">
                <x-input-label for="dept_id" :value="__('Department')" class="textlabel" />

                <select name="dept_id" id="dept_id" value="{{$tt['dept_id']}}">
            ...
            <div style="display:flex;flex-direction:row;width:100%;">
                <div style="display:flex;flex-direction:column;width:20%;">
                    ...
                    <input type="date" id="from" class="tfield" name="from_date" value="{{$tt['from_date']}}">
                </div>
                <div style="display:flex;flex-direction:column;width:20%;">
                    <label for="to_date" class="textlabel">To</label>
                    <input type="date" id="to" class="tfield" name="to_date" value="{{$tt['to_date']}}">
                </div>
            </div>
            <div style="display:flex;flex-direction:column;width:20%;">
                <label for="timet" class="textlabel">Upload Time Table</label>
                <input type="file" id="exampleInputPassword1" name="timet" style="color:white;" value="{{$tt['timet']}}">
            </div>
